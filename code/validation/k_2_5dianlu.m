...
beta = 2.0;     % 下层竖向线性刚度比
K1   = 1.0;     % 上层水平弹簧刚度比
K2   = 0.0;       % 下层水平弹簧刚度比 (默认0，第二组保持0，第一组将改为0.5)
U    = 2.0;     % 几何非线性尺度参数
L    = 4/9;     % QZS 长度比
...
% 修改第一组：将K2改为0.5，重新计算alpha2和gamma2
K2_new = 0.5;
alpha2_new = beta - 2*K2_new*(1-L)/L;          % 重新计算alpha2
gamma2_new = K2_new/(U^2 * L^3);                % 重新计算gamma2

P1.be2 = alpha2_new;   % 更新下层线性刚度比
P1.ga2 = gamma2_new;   % 更新下层非线性刚度系数

% 将两组参数放入元胞数组，便于循环处理
param_list = {P1, P2};
names = {'优化算子NIC电路 (\lambda=0.18, \kappa_e=1.82, \kappa_c=0.01, \sigma=0.54, K2=0.5)', ...
         '原始最优曲线 (\lambda=0, \kappa_e=0, \kappa_c=0, \sigma=0, K2=0)'};
colors = {'b', 'r'};   % 蓝色和红色
...
title(ax, 'BG Model 电路参数对比 (K2=0.5 for circuit case)');
yline(ax, 0, 'k--', '0 dB');
...
for k = 1:2
    ...
    fprintf('\n========================================\n');
    fprintf('开始计算第 %d 组: %s\n', k, names{k});
    fprintf('参数: lam=%.2f, kap_e=%.2f, kap_c=%.2f, sigma=%.2f, K2=%.1f\n', ...
            P.lam, P.kap_e, P.kap_c, P.sigma, (k==1)*0.5);   % 仅显示第一组的K2=0.5
